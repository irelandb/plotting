...
def gely(ax,df_annotation,annotation_spacing,fontsize=8,\
    ...
    for node in all_nodes:
        ...
        ya = num_rows - (node.first + node.offset + .1)
        yb = num_rows - (node.last + node.offset + .9)
        yc = 0.5*(ya+yb)
        plt.text(x-.6*annotation_spacing,yc,node.label,ha=ha,va='center',fontsize=fontsize,rotation=rotation)
        if len(node.children) > 1 and len(node.label) > 0:
