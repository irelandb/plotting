...
rnap_real_raw = pd.read_csv('GFP_compneural',delim_whitespace=True)
rnap_real_raw = rnap_real_raw[rnap_real_raw['Training/Test'].str.contains('rep' + sys.argv[1])]
...
rnap_real['model'] = ['nbr' if 'eighbor' in name else name[-3] for name in names]
...
plt.savefig('GFP_compneural_all_reps' + sys.argv[1] + '.pdf')
plt.close()
