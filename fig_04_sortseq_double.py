...
rnap_real_raw = pd.read_csv('double_comp2',delim_whitespace=True)
...
rnap_real['fitting'] = ['ER' if x=='ER' else x for x in rnap_real['fitting']]

# Reorder test datasets
rnap_real['label'] = '' #'RNAP model'
rnap_real = rnap_real[['label','training','model','fitting','single','double']]

#pdb.set_trace()

# Reorder list of models

indices = [('1' in name) for name in rnap_real['fitting']]
rnap_real = rnap_real[indices]

#rename columns
rename_list = []
for item in rnap_real['fitting']:
   if 'IM' in item:
       rename_list.append('IM')
   elif 'LS' in item:
       rename_list.append('LS')
   elif 'ER' in item:
       rename_list.append('ER')
rnap_real['fitting'] = rename_list
print rnap_real
...
crp_real['fitting'] = ['ER' if x=='ER' else x for x in crp_real['fitting']]
...
mat_er_indices = (rnap_real['model'] == 'mat') & (rnap_real['fitting'] == 'ER')
...
print rnap_real
print mat_im_indices
print M
print np.array(rnap_real[mat_im_indices].iloc[:,-M:])
...
mat_er_indices = (crp_real['model'] == 'mat') & (crp_real['fitting'] == 'ER')
...
plt.xlabel('$I_\mathrm{mat,ER}$',fontsize=labelsize)
plt.ylabel('$I_\mathrm{mat,IM}$',fontsize=labelsize)
plt.title('RNAP mat: IM vs. ER',fontsize=labelsize)
