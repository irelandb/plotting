...
rnap_real_raw = pd.read_csv('GFP_2bin_comp2_linfoot',delim_whitespace=True)
...
rnap_real.loc[93] = ['NeuralNet',.308,.322975,.239254,.263264,'all130NN','NN']
print rnap_real
# Reorder test datasets
rnap_real['label'] = '' #'RNAP model'
rnap_real = rnap_real[['label','training','model','fitting','rep0','rep1','rep2','rep3']]

#pdb.set_trace()

# Reorder list of models
indices = [(not 'iterLS' in name and '200' in name) for name in rnap_real['fitting']]
...
plt.savefig('GFP_compneural_2bins_linfoot.pdf')
plt.close()
